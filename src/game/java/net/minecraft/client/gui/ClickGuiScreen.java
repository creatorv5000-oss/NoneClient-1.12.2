...
private Category category = Category.Client {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        this.drawRect(x, y, x + 540, y + 360, 0xFA1E1E24); // Dark-charcoal background main window
        this.drawRect(x, y, x + 540, y + 42, 0xFF18181C);  // Upper branding navigation header bar

        // Brand Typography: Clean Lunar Client header text layout
        this.mc.fontRendererObj.drawString("Lunar Client", x + 16, y + 12, 0xFFFFFFFF);
        this.mc.fontRendererObj.drawString("(1.12.2 / NoneClient)", x + 88, y + 13, 0x66FFFFFF);

        // Search Input Box (Lunar right-aligned capsule style setup)
        int sx = x + 340;
        int sy = y + 10;
        this.drawRect(sx, sy, sx + 180, sy + 22, 0x44000000);
        this.drawRect(sx, sy + 21, sx + 180, sy + 22, searchFocused ? 0xFF3887FF : 0x11FFFFFF);
        ...
        this.mc.fontRendererObj.drawString(text, sx + 8, sy + 7, searchQuery.isEmpty() ? 0x44FFFFFF : 0xCCFFFFFF);
        ...
        for (Category c : Category.values()) {
            ...
            int stringWidth = this.mc.fontRendererObj.getStringWidth(c.name());

            // Render bottom highlight border lines underneath active categories
            if (selected) {
                this.drawRect(cx, cy + 18, cx + stringWidth, cy + 20, 0xFF3887FF);
            }
        ...
        this.drawRect(x, y + 74, x + 540, y + 75, 0x11FFFFFF);
        ...
        for (Module module : Client.manager.getModulesByCategory(category)) {
            ...
            this.drawRect(mx, my, mx + 240, my + 46, 0x44000000);
            this.drawRect(mx, my, mx + 3, my + 46, enabled ? 0xFF3887FF : 0x22FFFFFF);

            // Module Label Text strings
            this.mc.fontRendererObj.drawString(module.getName(), mx + 14, my + 10, enabled ? 0xFFFFFFFF : 0x99FFFFFF);

            String description = module.getDescription();
            if (description != null && !description.isEmpty() && !description.equals("- - -")) {
                this.mc.fontRendererObj.drawString(description, mx + 14, my + 24, 0x44FFFFFF);
            }

            // Slider toggle positioning parameters
            int tx = mx + 195;
            int ty = my + 16;
            this.drawRect(tx, ty, tx + 30, ty + 12, enabled ? 0xFF3887FF : 0x22FFFFFF);

            if (enabled) {
                this.drawRect(tx + 18, ty - 1, tx + 29, ty + 13, 0xFFFFFFFF);
            } else {
                this.drawRect(tx + 1, ty - 1, tx + 12, ty + 13, 0x55FFFFFF);
            }
